...
class crack_tracking_inverse(Node): # robot7
    def __init__(self):
        ...
        self.get_logger().info('crack_tracking_inverse node start!')
        ...
        self.danger_sub = self.create_subscription(Danger, '/danger', 
                                                      self.danger_callback, 10) # danger sub -> real-time detect
        self.exit_select_service = self.create_service(PersonFollow,'/person_follow',self.person_guide_start) #  servie create
        self.exit_reselect_client = self.create_client(PersonFollow,'/reperson_follow')
        self.pose_sub = self.create_subscription(PoseWithCovarianceStamped, '/robot7/amcl_pose', self.pose_callback,10)
        ...
        self.section_list = {0 : [-0.457, 6.21], # 4 exit
                        1 : [0.417, 5.28], # 5 exit
                        }
        # while not self.exit_reselect_client.wait_for_service(timeout_sec=2.0):
    ...
    def goal_result_callback(self, future):
        ...
        if result.status == GoalStatus.STATUS_SUCCEEDED:
            ...
            self.send_goal(-1.378, 3.102) # 6-7 robot 7
    ...
    def danger_callback(self,msg):
        self.get_logger().info(f'danger : {self.danger} /amcl : {self.current_pose} / self.person : {self.person}')
        self.danger = msg
    ...
    def follow_people(self): # loop per 2 second 
        # self.get_logger().info('follow_people function!')
        if self.person_detect == True: # following
            ...
            if (self.person.obstacle_distance < 2.0) and self.person.class_name =='car': # self.person.obstacle_distance > 1.0 -> plus?
                self.send_goal(self.section_list[self.select][0],self.section_list[self.select][1]) # exit position input
            elif self.person.obstacle_distance > 2.0 and self.person.class_name =='car': 
                while self.person.obstacle_distance < 2.0:# 사람이 인지가 되도 3.0m 이내가 아니면 제자리에서 기다리고
                ...
                #self.person initialize objectdetection value 
                    ...
                    self.person_detect =False #escape waypoint goal move -> signal detection
                    self.send_request() # repeople_follow request
            self.previous_pose = self.current_pose # resave waypoint
    #service_client
    def send_request(self): # service request
        self.get_logger().info('exit setting need')
        request = PersonFollow.Request()
        request.person_detect = False 
        futures = self.exit_reselect_client.call_async(request)
        futures.add_done_callback(self.response_callback)
        return futures
    
    def response_callback(self,futures): # result value get function callback
        response = futures.result().result # 'ok' arrived
        self.person_detect = True # response result setting
        
    # service_server
    def person_guide_start(self,request,response): # if exit guide request input 
        self.get_logger().info("person follow start!")
        section_x = request.x
        section_y = request.y
        self.human_callback(section_x,section_y) # exit select function -> self.select
        response.result = 'ok' # response
        self.person_detect = True # request.person_detect
        return response
    
    def human_callback(self, x,y): # distance calculator
        if self.person.class_name == 'car':
            try:
                # 사람 위치 정보
                human_x = float(x)
                human_y = float(y)

                # 출구 4와의 거리 계산self.section_list
                dx4 = human_x - self.section_list[0][0]
                dy4 = human_y - self.section_list[0][1]
                dist_exit4 = (dx4 ** 2 + dy4 ** 2) ** 0.5

                # 출구 5와의 거리 계산
                dx5 = human_x - self.section_list[1][0]
                dy5 = human_y - self.section_list[1][1]
